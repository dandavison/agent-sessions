...
from senderos import db, topology
...
def build(conn, nodes: list[Node], leaf: str | None = None, **kw) -> dict:
    db.write_sendero(
        conn,
        Sendero(id=SID, agent="claude", native_id="a", path="/t/a.jsonl", leaf_uuid=leaf, **kw),
    )
...
def test_a_straight_thread_is_one_segment(conn) -> None:
    sendero = build(conn, [node("u1", None, 0), node("a1", "u1", 1), node("u2", "a1", 2)], "u2")
    (root,) = topology.of(conn, sendero).roots
...
def test_the_branch_leading_to_the_leaf_was_taken(conn) -> None:
    sendero = build(conn, [node("u1", None, 0), node("x", "u1", 1), node("y", "u1", 2)], "x")
    (root,) = topology.of(conn, sendero).roots
...
def test_with_no_leaf_among_them_the_last_written_was_taken(conn) -> None:
    ...
    first_era = topology.of(conn, sendero).roots[0]
...
def test_compaction_starts_a_root_of_its_own(conn) -> None:
    ...
    sendero = build(conn, [node("u1", None, 0), node("c1", None, 1), node("u2", "c1", 2)], "u2")
    ...
    roots = topology.of(conn, sendero).roots
    assert len(roots) == 2
...
def test_context_of_a_segment_is_its_last_known_value(conn) -> None:
    ...
    sendero = build(conn, nodes, "a2")
    assert topology.of(conn, sendero).roots[0].context_tokens == 5000


def test_forks_out_are_listed(conn) -> None:
    sendero = build(conn, [node("u1", None, 0)], "u1")
    db.write_edges(conn, [Edge(child="claude:b", parent=SID, kind="fork", at_uuid="u1")])
    conn.commit()
    assert topology.of(conn, sendero).forks == [{"child": "claude:b", "at_uuid": "u1"}]
...
def test_the_fork_it_came_from_is_named(conn) -> None:
    ...
    conn.commit()
    assert topology.of(conn, sendero).forked_from == {"parent": "claude:parent", "at_uuid": "u1"}


def test_a_sendero_with_no_forks(conn) -> None:
    sendero = build(conn, [node("u1", None, 0)], "u1")
...
def test_a_node_whose_parent_is_missing_starts_a_root(conn) -> None:
    ...
    sendero = build(conn, [node("u1", None, 0), node("orphan", "gone", 1)], "orphan")
    assert [r.start for r in topology.of(conn, sendero).roots] == ["u1", "orphan"]
